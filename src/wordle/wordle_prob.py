...
def compare(word, key):
    ...
    for letter in string.ascii_lowercase:
        if letter not in word or letter not in key:
            continue
...
def main():
    ...
    while True:
        ...
        if com[0] in ["exit", "quit", "end"]:
            ...
            if (len(pruned) > 40 or com[-1] == "c") and com[-1] != "f":
                print(f"There are {len(pruned)} words")
            else:
                print("\n".join(word.word for word in pruned))
        elif com[0] in ["best", "besth"]:
            scores = {
                word: score(word.word, pruned)
                for word in {"best": words, "besth": pruned}[com[0]]
            }
